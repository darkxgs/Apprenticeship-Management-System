...
/**
 * تصفية طلاب الدور الثاني لإخراج تسويدة رصد درجات الدور الثاني.
 * يعرض فقط الحالات التي تدخل الدور الثاني (نفس حالات صفحة رصد الدور الثاني):
 * دور ثاني / مؤجل / ناجح دور ثاني.
 */
public class EltaSoedaSecondRound extends javax.swing.JFrame {

    private static final java.util.logging.Logger logger = java.util.logging.Logger
            .getLogger(EltaSoedaSecondRound.class.getName());

    public EltaSoedaSecondRound() {
        initComponents();
        setupTableUi();
        loadFilters();
        setTitle("تصفية الطلاب - تسويدة الدور الثاني");
    ...
    private void loadStudents(String region, String center, String profession) {
        ...
        // طلاب الدور الثاني فقط (نفس حالات صفحة رصد درجات الدور الثاني)
        sql.append("AND TRIM(status) IN ('دور ثاني', 'مؤجل', 'ناجح دور ثاني') ");
    ...
    private void initComponents() {
        ...
        btnTasoeda.setText("إخراج تسويدة الدور الثاني");
        ...
        jLabel1.setText("تصفية الطلاب - تسويدة الدور الثاني");
        gridBagConstraints = new java.awt.GridBagConstraints();
    ...
    private void generateReport() {
        ...
        ReportWorker worker = new ReportWorker(this, "تسويدة رصد درجات الدور الثاني", null) {
            @Override
            protected Void doInBackground() throws Exception {
                ...
                try (Connection con = DatabaseConnection.getConnection()) {
                    ...
                    java.io.File folder = new java.io.File("التقارير/تسويدة الدور الثاني");
                    ...
                    for (java.util.Map.Entry<String, java.util.List<Student>> entry : groupedByProfession.entrySet()) {
                        ...
                        String combinedFn = "التقارير/تسويدة الدور الثاني/" + sanitizedProfession + ".pdf";
                        com.itextpdf.text.pdf.PdfWriter.getInstance(combinedDoc, new java.io.FileOutputStream(combinedFn));
                        combinedDoc.open();

                        updateStatus(processed, totalSelected, "جاري إنشاء تسويدة الدور الثاني لمهنة: " + professionName);

                        String rName = !professionStudents.isEmpty() ? professionStudents.get(0).getRegion() : "";

                        gradReportTasoedaSecondRound report = new gradReportTasoedaSecondRound(professionName, displayCenterName, rName, professionStudents, false,
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> new EltaSoedaSecondRound().setVisible(true));
    }
