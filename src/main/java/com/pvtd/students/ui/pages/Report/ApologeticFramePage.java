...
public class ApologeticFramePage extends javax.swing.JFrame {
    ...
    public ApologeticFramePage() {
        ...
        this.addWindowFocusListener(new java.awt.event.WindowFocusListener() {
            ...
            public void windowGainedFocus(java.awt.event.WindowEvent e) {
                ...
                String region = (String) combobox2.getSelectedItem();
    ...
    private javax.swing.JPanel createStatsPanel() {
        ...
        statsContainer.add(createStatCard("إجمالي الطلاب", lblStatTotal, new java.awt.Color(79, 70, 229))); 
        statsContainer.add(createStatCard("الطلاب المعروضين", lblStatFiltered, new java.awt.Color(67, 56, 202))); 
        statsContainer.add(createStatCard("الطلاب المحددين", lblStatSelected, new java.awt.Color(55, 48, 163))); 
    ...
    private void modernizeComponents() {
        ...
        buttonGradient2.setText("📄 عرض بدون درجات");
        buttonGradient2.setColor1(new java.awt.Color(79, 70, 229));
        buttonGradient2.setColor2(new java.awt.Color(55, 48, 163));
        buttonGradient2.setFont(UITheme.FONT_HEADER);
        buttonGradient2.setRadius(20);
        
        buttonGradientGrades.setText("📊 كشف بالدرجات");
        buttonGradientGrades.setColor1(new java.awt.Color(67, 56, 202));
        buttonGradientGrades.setColor2(new java.awt.Color(49, 46, 129));
    ...
    private void setupTableUi() {
        ...
        // ضبط عرض الأعمدة: حالة الطالب، رقم الجلوس، رقم التسجيل، المهنه، الاسم، م
        javax.swing.SwingUtilities.invokeLater(() -> {
            jTable1.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_OFF);
            int[] colWidths = { 100, 100, 120, 300, 220, 50 };
            for (int i = 0; i < colWidths.length && i < jTable1.getColumnCount(); i++) {
                jTable1.getColumnModel().getColumn(i).setPreferredWidth(colWidths[i]);
            }
    ...
    public void loadRegions() {
        ...
        combobox2.addItem("اختر المنطقة...");
    ...
    private void initComponents() {
        ...
        combobox2 = new com.pvtd.students.ui.components.Combobox();
        ...
        jPanel2.setPreferredSize(new java.awt.Dimension(618, 100));

        // 1. Title Label
        jLabel1.setFont(new java.awt.Font("Segoe UI", 1, 20));
        jLabel1.setForeground(new java.awt.Color(255, 255, 255));
        jLabel1.setText("تقارير الطلاب المعتذرين");
        ...
        jPanel2.add(combobox2, gridBagConstraints);
        ...
        buttonGradient2.setText("📄 بدون درجات");
        ...
        layout.setVerticalGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addComponent(jPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, 649, Short.MAX_VALUE));
        pack();
    }// </editor-fold>

    private void cmdcenterActionPerformed(java.awt.event.ActionEvent evt) {
        if (cmdcenter.getSelectedItem() != null && combobox2.getSelectedItem() != null) {
    ...
    private void buttonGradient2ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        Apologetic report = new Apologetic(selMonth, admMonth);
        if (report.isCancelled) return;
        DefaultTableModel model1 = (DefaultTableModel) jTable1.getModel();
        ReportWorker worker = new ReportWorker(this, "كشف الطلاب المعتذرين", null) {
            @Override
            protected Void doInBackground() throws Exception {
                ...
                try (Connection con = DatabaseConnection.getConnection()) {
                    // Unified Logic: Fetch system directly from DB using JOIN with professions
                    String inClause = java.util.Collections.nCopies(selectedRows.length, "?").stream().collect(java.util.stream.Collectors.joining(","));
                    String sql = "SELECT s.seat_no, s.exam_system " +
                                 "FROM students s " +
                                 "WHERE s.seat_no IN (" + inClause + ")";
                    
                    java.util.Map<String, String> seatToSystem = new java.util.HashMap<>();
                    try (PreparedStatement ps = con.prepareStatement(sql)) {
                        for (int i = 0; i < selectedRows.length; i++) {
                            ps.setString(i + 1, String.valueOf(model1.getValueAt(selectedRows[i], 1))); 
                        }
                        try (ResultSet rs = ps.executeQuery()) {
                            while (rs.next()) {
                                String sys = rs.getString("exam_system");
                                if (sys == null) sys = "";
                                seatToSystem.put(rs.getString("seat_no"), sys); 
                            }
                        }
                    }
                    ...
                    for (int i = 0; i < selectedRows.length; i++) {
                        ...
                        // Note: Getting system name from our unified DB fetch
                        String seatNo = String.valueOf(model1.getValueAt(selectedRows[i], 1));
                        // Re-fetch systemName based on seat_no (or we could fetch all student data here)
                        // But since we need to match the rowData in jTable, we'll use a seatNo map.
                        String sysFromDb = "";
                        // Re-query or refine: best to just use the joined data we got.
                        ...
                        // Using the same logic as Grades report to find the system
                        String systemName = "";
                        try (PreparedStatement psSys = con.prepareStatement("SELECT s.exam_system FROM students s WHERE s.seat_no = ?")) {
    ...
    private void buttonSecretReportActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        ReportWorker worker = new ReportWorker(this, "كشف المعتذرين بالدرجات", null) {
            ...
            protected Void doInBackground() throws Exception {
                ...
                try (Connection con = DatabaseConnection.getConnection()) {
                    ...
                    for (int i = 0; i < selectedSeatNos.size(); i += batchSize) {
                        ...
                        String sql = "SELECT s.id, s.name, s.registration_no, s.seat_no, s.status, s.national_id, s.profession, s.professional_group, s.secret_no, s.coordination_no, s.region, s.exam_system " +
                                     "FROM students s " +
                                     "WHERE s.seat_no IN (" + inClause + ")";
                        try (PreparedStatement ps = con.prepareStatement(sql)) {
                            ...
                            try (ResultSet rs = ps.executeQuery()) {
                                ...
                                st.setExamSystem(sys);
                                    ...
                                    if (r == null || r.trim().isEmpty()) {
                                        r = (combobox2.getSelectedItem() != null) ? combobox2.getSelectedItem().toString() : "";
                                    }
                                    st.setRegion(r);
                    ...
                    for (String seatNo : selectedSeatNos) {
                        ...
                        if (st != null) {
                            ...
                            String sys = st.getExamSystem();
                            if (sys == null) sys = "";
                            
                            studentsByRegionAndSystem
                                .computeIfAbsent(reg, k -> new java.util.LinkedHashMap<>())
                                .computeIfAbsent(sys, k -> new java.util.ArrayList<>())
                                .add(st);
                        }
                    }

                    generatePdfFiles(studentsByRegionAndSystem, centerName, selMonth, admMonth, "التقارير/تبييضة/معتذرين");
                }
                java.awt.Desktop.getDesktop().open(new File("التقارير/تبييضة/معتذرين"));
                return null;
            }
            ...
            private void generatePdfFiles(java.util.LinkedHashMap<String, java.util.LinkedHashMap<String, java.util.List<com.pvtd.students.models.Student>>> data, 
                ...
                // Aggregating all students into one center-wide list
                ...
                for (java.util.Map.Entry<String, java.util.LinkedHashMap<String, java.util.List<com.pvtd.students.models.Student>>> regEntry : data.entrySet()) {
                    if (mainRegion.isEmpty() && regEntry.getKey() != null) mainRegion = regEntry.getKey();
                    for (java.util.List<com.pvtd.students.models.Student> sysList : regEntry.getValue().values()) {
                        fullCenterStudents.addAll(sysList);
                    }
                }

                if (!fullCenterStudents.isEmpty()) {
                    // Sort all students numerically by seat number
                    ...
                    gradReportSequential report = new gradReportSequential("تلاميذ معتذرون", new java.awt.Color(0, 153, 76), center, mainRegion, fullCenterStudents, selM, admM);
    ...
    private com.pvtd.students.ui.components.Combobox combobox2;
