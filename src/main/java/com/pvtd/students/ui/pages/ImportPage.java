...
import com.pvtd.students.services.ImageImportService;
...
public class ImportPage extends JPanel {
    ...
    public ImportPage(AppFrame frame) {
        ...
        centerSection.add(Box.createVerticalStrut(12));

        // Image Only Upload Button
        JButton imageOnlyBtn = new JButton("🖼️  استيراد صور فقط (بدون إكسل)");
        imageOnlyBtn.setFont(UITheme.FONT_HEADER);
        imageOnlyBtn.setForeground(UITheme.PRIMARY);
        imageOnlyBtn.setBackground(Color.WHITE);
        imageOnlyBtn.setAlignmentX(Component.CENTER_ALIGNMENT);
        imageOnlyBtn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        imageOnlyBtn.putClientProperty("JButton.buttonType", "roundRect");
        imageOnlyBtn.setPreferredSize(new Dimension(340, 48));
        imageOnlyBtn.setMaximumSize(new Dimension(400, 48));
        imageOnlyBtn.setBorder(BorderFactory.createLineBorder(UITheme.PRIMARY, 1, true));
        imageOnlyBtn.addActionListener(e -> handleImageOnlyImport());
        centerSection.add(imageOnlyBtn);
    ...
    private void handleImageOnlyImport() {
        JFileChooser chooser = new JFileChooser();
        chooser.setDialogTitle("اختر مجلد صور الطلاب للمطابقة بالرقم القومي");
        chooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
        ...
        progressBar.setIndeterminate(false);
        progressBar.setValue(0);
        statusLabel.setText("جاري تحليل مجلد الصور والمطابقة مع قاعدة البيانات...");

        final String username = frame != null ? frame.getLoggedInUser().getUsername() : "SYSTEM";

        SwingWorker<ImageImportService.ImageImportResult, String> worker = new SwingWorker<ImageImportService.ImageImportResult, String>() {
            @Override
            protected ImageImportService.ImageImportResult doInBackground() {
                return ImageImportService.importImagesFromFolder(folder, username,
                        (current, total, msg) -> {
                            int pct = (int) (((double) current / total) * 100);
                            setProgress(pct);
                            publish(msg + " (" + current + "/" + total + ")");
            ...
            protected void done() {
                ...
                progressBar.setValue(100);
                try {
                    ImageImportService.ImageImportResult result = get();
                    if (result != null) {
                        StringBuilder report = new StringBuilder();
                        report.append("--- تقرير استيراد الصور ---\n");
                        report.append("✅ عدد الطلاب الذين تم ربط صورهم بنجاح: ").append(result.linkedCount).append("\n");
                        report.append("❓ عدد الصور التي لم يتم العثور على طالب مطابق لها: ").append(result.unmatchedImagesCount).append("\n");
                        report.append("⚠️ عدد الطلاب الذين لم يتم ربط أي صورة لهم: ").append(result.studentsWithoutImagesCount).append("\n");
                        
                        if (!result.errors.isEmpty() || !result.unmatchedImageNames.isEmpty()) {
                            report.append("\n--- التفاصيل والأخطاء ---\n");
                            for (String err : result.errors) {
                                report.append("❌ ").append(err).append("\n");
                            }
                            for (String unm : result.unmatchedImageNames) {
                                report.append("- ").append(unm).append("\n");
                            }
                        }
                        ...
                        statusLabel.setText("✅ اكتملت عملية استيراد الصور.");
                        JOptionPane.showMessageDialog(ImportPage.this, scroll, "تقرير عملية الاستيراد", JOptionPane.INFORMATION_MESSAGE);
                        
                    } else {
                        statusLabel.setText("❌ فشلت عملية الاستيراد.");
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
