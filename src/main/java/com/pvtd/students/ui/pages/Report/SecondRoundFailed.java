 ...
 * @author Seif
 */
public class SecondRoundFailed extends javax.swing.JFrame {
	private String manualExamMonth = null;
	private String manualAdmissionMonth = null;
	public boolean isCancelled = false;

	private static final java.util.logging.Logger logger = java.util.logging.Logger
			.getLogger(SecondRoundFailed.class.getName());

	/**
	 * Creates new form Failed
	 */
	public SecondRoundFailed() {
		initComponents();
		initTableLogic();
	}

	public SecondRoundFailed(String examMonth, String admissionMonth) {
	...
	private void initTableLogic() {
		...
		jTable2.getColumnModel().getColumn(0).setPreferredWidth(500); // مواد الرسوب
		...
		jTable2.getColumnModel().getColumn(4).setPreferredWidth(400); // المهنة
		jTable2.getColumnModel().getColumn(5).setPreferredWidth(400); // الاسم
		...
		javax.swing.table.DefaultTableCellRenderer centerCellRenderer = new javax.swing.table.DefaultTableCellRenderer() {
			...
			public Component getTableCellRendererComponent(JTable table, Object value,
				...
				int fontSize = 23; // Increased base font for better visibility
				// Columns 4 (المهنة) and 5 (الاسم)
				if (column == 4 || column == 5) {
					...
					if (colWidth > 0 && !txt.isEmpty()) {
						// Measure text and shrink font if necessary
						...
						while (fontSize > 10) {
							...
							if (textWidth < colWidth - 25) // Increased safety margin to 25
								break;
							fontSize--;
						}
						c.setFont(f);
					...
					// Removed fixed width to prevent artificial clipping; using rtl and right padding
		...
		jLabel6.setText("تلاميذ راسبون من الدور الثاني");
	...
	private String[] getFailedSubjectsForSeat(String seatNo) {
		...
		try (Connection con = DatabaseConnection.getConnection()) {
			...
			// Get subjects for this profession (parent subjects only, with pass_mark and
			// type)
	...
	public void buildPagePanel(int rowCount) {
		...
		// rowH = remaining space after header, divided by 30 rows
		int rowH = (tableHeight - tableHeaderH) / 30;
		// Capped at 50px to ensure fit within page
	...
	public void createPDF() {
		try {
			...
			int rowsPerPage = 30; // Updated to 30 students per page
			...
			for (int page = 0; page < pageCount; page++) {
				...
				for (int i = start; i < end; i++) {
					...
					row.set(0, "راسب من الدور الثاني"); // 0: حالة التلميذ
					row.set(5, globalIndex++); // 5: م
					model.addRow(row);
				}

				// Fill remaining rows to 30 so table always fills the page
	...
	public void createPDFGroupedBySystem(
		...
		try {
			...
			String folderStr = "التقارير/بدون درجات/راسبين_دور_ثاني";
			...
			for (int page = 0; page < pageCount; page++) {
				...
				for (int i = start; i < end; i++) {
					...
					newRow[1] = "راسب من الدور الثاني"; // حالة
