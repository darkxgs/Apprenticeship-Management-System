...
public class sucsseccFromPage extends javax.swing.JFrame {
    ...
    public void loadStudentInfo(String seatNo, Connection con) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (specialization != null && !specialization.trim().isEmpty() && !specialization.equals(rs.getString("profession"))) {
                    specLbl.setText(rs.getString("profession") + " / " + specialization);
                } else {
                    specLbl.setText(rs.getString("profession"));
                }
            }
    ...
    public void loadStudentSubjects(String seatNo, Connection con) {
        try {
            // Improved query: Select subjects for the student's profession and join with
            // grades.
            // Using COALESCE(p.display_order, sub.display_order) ensures children (30/70)
            // stay in the same position as their parent, respecting the specialized theory
            // (1 & 2) order.
            String sql = """
            ...
            for (SubData sd : allDetails) {
                if (sd.parentId != null)
                    parentsWithChildren.add(sd.parentId);
            }
            ...
            for (SubData sd : allDetails) {
                ...
                if (groupedMap.containsKey(key)) {
                    ...
                    if (isParentOfComponents) {
                        ...
                        groupedMap.put(key,
                                new SubData(sd.name, sd.max, sd.pass, sd.mark, sd.type, sd.parentId, sd.id));
                    }
                }
            }
