...
public class DataEntryPage extends JPanel {
    ...
    private JPanel buildTopBar() {
        ...
        secretNoField.setEditable(true); // Allow typing for search
        ...
        secretNoField.addActionListener(e -> searchBySecret()); // Trigger search on Enter
    ...
    private void searchBySecret() {
        String num = secretNoField.getText().trim();
        if (num.isEmpty()) {
            num = JOptionPane.showInputDialog(this, "أدخل الرقم السري للبحث:", "بحث بالرقم السري", JOptionPane.QUESTION_MESSAGE);
        }
    ...
    private void performGlobalSecretSearch(String secretNo) {
        ...
        String regionName = s.getRegion();
        String regionDisplay = null;
        if (regionName != null) {
            for (Map.Entry<String, String> entry : regionCodeToNameMap.entrySet()) {
                if (entry.getValue().equals(regionName)) {
                    regionDisplay = entry.getKey();
                    break;
                }
            }
        }
        ...
        if (regionDisplay != null) {
            regionCombo.setSelectedItem(regionDisplay);
        } else {
            regionCombo.setSelectedIndex(0); // Fallback to "All Regions"
        }
        
        // This repopulates the centerCombo
        onRegionSelected();

        // 2. Identify and select correct Center
        String centerName = s.getCenterName();
        String centerDisplay = null;
        if (centerName != null) {
            for (Map.Entry<String, String> entry : centerCodeToNameMap.entrySet()) {
                if (entry.getValue().equals(centerName)) {
                    centerDisplay = entry.getKey();
                    break;
                }
            }
        }
        ...
        if (centerDisplay != null) {
            centerCombo.setSelectedItem(centerDisplay);
            // This triggers onCenterSelected() which loads centerStudents and navigates to 0
        } else {
            // If center not found in current region's centers (e.g. if we went to "All Regions")
            // Try to set it by forcing it if we can find it in the map
            // For simplicity, we assume the center exists in the loaded list
        }
    ...
    private void renderSubjectsForProfession() {
        ...
        for (Subject sub : currentSubjects) {
            if (sub.getParentSubjectId() != null) {
                childrenMap.computeIfAbsent(sub.getParentSubjectId(), k -> new ArrayList<>()).add(sub);
                childIds.add(sub.getId());
            }
        }
        ...
        // Only add top-level subjects (not children) to their type lists
        for (Subject sub : currentSubjects) {
            if (childIds.contains(sub.getId())) continue; // skip sub-subjects
    ...
    private void renderSubjectGroup(JPanel container, String title, Color headerBg,
                                    ...
                                    java.util.Map<Integer, List<Subject>> childrenMap) {
        ...
        for (Subject sub : subjects) {
            List<Subject> children = childrenMap.get(sub.getId());

            if (children != null && !children.isEmpty()) {
                // ── Composite (30/70) subject card ──────────────────────────
                RoundedPanel compositeCard = new RoundedPanel(10, new Color(0xF0FDF4));
                compositeCard.setLayout(new BoxLayout(compositeCard, BoxLayout.Y_AXIS));
                compositeCard.setBorder(BorderFactory.createCompoundBorder(
                    new LineBorder(CLR_TEAL_DARK, 1, true),
                    new EmptyBorder(8, 10, 8, 10)));
                compositeCard.setAlignmentX(Component.LEFT_ALIGNMENT);
                compositeCard.setMaximumSize(new Dimension(Integer.MAX_VALUE, 200));

                // Parent label (main subject name)
                JLabel parentLbl = new JLabel(sub.getName() + " /مجموع: " + sub.getMaxMark() + "/", SwingConstants.RIGHT);
                parentLbl.setFont(new Font("Segoe UI", Font.BOLD, 14));
                parentLbl.setForeground(new Color(0x065F46));
                parentLbl.setAlignmentX(Component.RIGHT_ALIGNMENT);
                compositeCard.add(parentLbl);
                compositeCard.add(Box.createVerticalStrut(6));

                // Each child as its own row inside the composite card
                for (Subject child : children) {
                    JPanel row = new JPanel(new BorderLayout(8, 0));
                    row.setOpaque(false);
                    row.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);

                    String subLabel = (child.getSubName() != null && !child.getSubName().isEmpty())
                        ? child.getSubName() + " /" + child.getMaxMark() + "/"
                        : child.getName() + " /" + child.getMaxMark() + "/";

                    RoundedPanel lblPill = new RoundedPanel(10, new Color(0xD1FAE5));
                    lblPill.setLayout(new BorderLayout());
                    lblPill.setPreferredSize(new Dimension(160, 36));
                    JLabel lbl = new JLabel(subLabel, SwingConstants.CENTER);
                    lbl.setFont(new Font("Segoe UI", Font.BOLD, 13));
                    lbl.setForeground(new Color(0x065F46));
                    ...
                    gradeInput.setPreferredSize(new Dimension(50, 36));
                    gradeInput.setBorder(new LineBorder(CLR_TEAL_DARK, 1, true));
                    gradeInput.setForeground(CLR_DARK_BLUE);

                    setupGradeInput(gradeInput, child.getMaxMark());
                    gradeFieldsMap.put(child.getId(), gradeInput);
                    orderedGradeFields.add(gradeInput);

                    row.add(lblPill, BorderLayout.CENTER);
                    row.add(gradeInput, BorderLayout.EAST);
                    compositeCard.add(row);
                    compositeCard.add(Box.createVerticalStrut(4));
                }

                // Wrapper to add left/right margins
                JPanel wrapper = new JPanel(new BorderLayout());
                wrapper.setOpaque(false);
                wrapper.setBorder(new EmptyBorder(0, 10, 10, 10));
                wrapper.add(compositeCard, BorderLayout.CENTER);
