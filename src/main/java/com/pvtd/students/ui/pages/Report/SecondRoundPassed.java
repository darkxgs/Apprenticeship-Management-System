...
public class SecondRoundPassed extends javax.swing.JFrame {
    private String manualExamMonth = null;
    private String manualAdmissionMonth = null;
    public boolean isCancelled = false;

    private static final java.util.logging.Logger logger = java.util.logging.Logger
            .getLogger(SecondRoundPassed.class.getName());

    public SecondRoundPassed() {
        initComponents();
        initTableLogic();
    }

    public SecondRoundPassed(String examMonth, String admissionMonth) {
    ...
    private void initTableLogic() {
        ...
        header.setDefaultRenderer(new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value,
                ...
                c.setFont(new Font("Tahoma", Font.BOLD, 24));
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            ...
            while (rs.next()) {
                ...
                model.addRow(new Object[] {
                        "ناجح من الدور الثاني", // 0: حالة التلميذ (تظهر في اليسار)
    ...
    public void buildPagePanel(int rowCount) {
        ...
        // Region, Center, System before ministry
        ...
        jLabel6.setFont(new Font("Tahoma", Font.BOLD, 30)); // تلاميذ ناجحون من الدور الثاني
        ...
        // Fixed full-page height for 32 rows (32*50 + header space)
        ...
        try {
            ...
            for (int page = 0; page < pageCount; page++) {
                ...
                for (int i = start; i < end; i++) {
                    ...
                    row.set(0, "ناجح من الدور الثاني");
    ...
    public void createPDFGroupedBySystem(
        ...
        try {
            ...
            String folderStr = "التقارير/بدون درجات/ناجحين_دور_ثاني";
            ...
            String filePath2 = folderStr + "/" + sanitizedCenter + ".pdf";

            Document document = new Document(PageSize.A4, 0, 0, 0, 0);
            PdfWriter.getInstance(document, new FileOutputStream(filePath2));
            document.open();

            // Flatten all rows into a single list while keeping track of current system for header updates
            ...
            for (int page = 0; page < pageCount; page++) {
                ...
                // Use the system of the first student on this page for the header
                String currentSys = (end > start) ? systemsForRow.get(start) : "";
                system.setText(currentSys);
                cent.setText(centerName);
                regoin.setText(regionName);

                for (int i = start; i < end; i++) {
                    java.util.Vector row = new java.util.Vector(allRows.get(i));
                    row.set(0, "ناجح من الدور الثاني");
                    row.set(5, globalIndex++);
                    model.addRow(row);
                }
    ...
    public void loadCenterData(String centerName) {
        try (Connection con = DatabaseConnection.getConnection()) {
            String sql = """
                    SELECT DISTINCT s.region, s.exam_system
                    FROM students s
                    WHERE s.center_name = ?
                    AND ROWNUM = 1
                    """;
    ...
    private void initComponents() {
        ...
        jLabel6.setText("تلاميذ ناجحون من الدور الثاني");
        jPanel1.add(jLabel6);
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new SecondRoundPassed().setVisible(true));
    }
