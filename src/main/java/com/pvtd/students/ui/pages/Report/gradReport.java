    ...
    private JScrollPane buildTable() {
        ...
        table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object val, boolean sel, boolean foc, int row, int col) {
                ...
                Component c = super.getTableCellRendererComponent(t, arTxt, sel, foc, row, col);
                if (row == 0) {
                ...
                // Auto-shrink font
                int baseSize = 14, minSize = 8;
                int colW = t.getColumnModel().getColumn(col).getWidth() - 10;
                Font useFont = new Font("Segoe UI", (row == 0 || row == 1) ? Font.BOLD : Font.PLAIN, baseSize);
                if (colW > 0 && !plain.isEmpty()) {
                    java.awt.FontMetrics fm = t.getFontMetrics(useFont);
                    int sz = baseSize;
                    while (fm.stringWidth(plain) > colW && sz > minSize) {
                        sz--;
                        useFont = new Font("Segoe UI", (row == 0 || row == 1) ? Font.BOLD : Font.PLAIN, sz);
