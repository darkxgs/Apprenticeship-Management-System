...
/**
 * Report: تلاميذ ناجحون — dynamic subject columns + max/min rows
 */
public class gradReportSucc extends JFrame {
    ...
    public gradReportSucc(String profession, List<Student> students) {
    ...
    private JPanel buildHeader() {
        ...
        JLabel sub = new JLabel("تلاميذ ناجحون", SwingConstants.CENTER);
    ...
    private JScrollPane buildTable() {
        ...
        // Columns (RTL): م, الاسم, كود الحرفه, الحرفه, الرقم القومي, رقم الجلوس,
        //   [subjects...], مجموع النظري, درجات العملي, درجات التطبيقي,
        //   مجموع العملي والتطبيقي, المجموع الكلي, حاله التلميذ
        String[] cols = new String[6 + subCount + 6];
        int i = 0;
        cols[i++] = "م";
        cols[i++] = "الاسم";
        cols[i++] = "كود الحرفه";
        ...
        for (int j = 0; j < subjects.size(); j++) {
            maxRow[6 + j] = subjects.get(j).getMaxMark();
        }
        model.addRow(maxRow);
        ...
        for (int j = 0; j < subjects.size(); j++) {
            minRow[6 + j] = subjects.get(j).getPassMark();
        }
        model.addRow(minRow);
        ...
        for (Student st : students) {
            ...
            for (Subject sub : subjects) {
                int mark = st.getGrades() != null ? st.getGrades().getOrDefault(sub.getId(), 0) : 0;
                row[colIdx++] = mark != 0 ? mark : "";
                if (sub.getType().equals("نظري")) {
        ...
        table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object val, boolean sel, boolean foc, int row, int col) {
                ...
                if (row == 0) {
                    c.setBackground(new Color(0xDCFCE7));  // green-ish for max
                    c.setFont(c.getFont().deriveFont(Font.BOLD));
                } else if (row == 1) {
                    c.setBackground(new Color(0xFEF9C3));  // yellow for min
                    c.setFont(c.getFont().deriveFont(Font.BOLD));
                } else {
                    c.setBackground(row % 2 == 0 ? Color.WHITE : new Color(0xF0FFF4));
                }
    ...
    private void styleTable(JTable table) {
        ...
        table.getTableHeader().setBackground(new Color(0x0A192F));
