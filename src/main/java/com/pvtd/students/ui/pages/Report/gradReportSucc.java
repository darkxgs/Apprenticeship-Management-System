...
/**
 * Report: تلاميذ ناجحون — Precise Layout with signature footer fixed immediately below the 50th row.
 */
    ...
    // Row height is now calculated dynamically inside buildPagePanel
    ...
    private JPanel buildHeader(int pageNum, int totalPages) {
        ...
        JLabel sub = label("تلاميذ ناجحون", 24, true);
    ...
    private JPanel buildTable(List<Student> chunk) {
        ...
        for (Subject s : parentSubjects) {
            ...
            maxRow[subIdx++] = m;
            totalMax += m;
            if ("نظري".equals(s.getType())) theoryMaxSum += m;
            else if ("تطبيقي".equals(s.getType())) appliedMax = m;
            else practicalMax = m;
        }
        ...
        for (Subject s : parentSubjects) {
            ...
            minRow[subIdx++] = p;
            if ("نظري".equals(s.getType())) theoryPassSum += p;
            else if ("تطبيقي".equals(s.getType())) appliedPass = p;
            else practicalPass = p;
        }
        ...
        for (int i_chunk = 0; i_chunk < 20; i_chunk++) {
            if (i_chunk < chunk.size()) {
                ...
                for (Subject s : parentSubjects) {
                    ...
                    if (children != null) {
                        for (Subject c : children) mark += (grades != null ? grades.getOrDefault(c.getId(), 0) : 0);
                    }
                    // Show '0' for any non-positive mark (including absent -1)
                    row[colIdx++] = mark > 0 ? mark : "0";
                    if ("نظري".equals(s.getType())) theorySum += (mark > 0 ? mark : 0);
                    else if ("تطبيقي".equals(s.getType())) appliedMark = (mark > 0 ? mark : 0);
        ...
        table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object val, boolean sel, boolean foc, int row, int col) {
                ...
                // Tahoma is very clear for Arabic. Size 22 is optimal.
    ...
    private JPanel buildPagePanel(List<Student> chunk, int pageNum, int totalPages) {
        ...
        // Fixed A3 landscape dimensions at high resolution (Ratio ~1.414)
        int panelWidth = 2800;
        int panelHeight = (int) (panelWidth / 1.4142); // ~1980px
        
        // Calculate dynamic row height to fill space exactly
        // Header ~220, Footer ~110, TableHeader ~150
        int headerH = 220, footerH = 110, tableHeaderH = 150;
        int available = panelHeight - headerH - footerH - tableHeaderH - 20; 
        int totalRows = 20 + 2; // +2 for Ends rows
    ...
    public void appendToDocument(Document doc) {
        try {
            ...
            for (int pIdx = 0; pIdx < totalPages; pIdx++) {
                ...
                // Use A3 Landscape as standard for these reports
                ...
                // Fill the entire A3 page completely
