...
/**
 * Report: تلاميذ ناجحون — Precise Layout synchronized with failed report
 * style (A3 Landscape).
 */
public class gradReportSucc extends JFrame {
    ...
    private void initUI() {
        ...
        // Standardized to 20 rows per page
        int pageSize = 15;
        ...
        uiRootPanel.add(buildTable(students.subList(0, Math.min(pageSize, students.size())), 1));
    ...
    private JPanel buildHeader(int pageNum, int totalPages) {
        ...
        JLabel t2 = label("تلاميذ ناجحون", 160, true);
    ...
    private JPanel buildTable(List<Student> chunk, int pageNum) {
        ...
        for (int i_chunk = 0; i_chunk < 15; i_chunk++) {
            if (i_chunk < chunk.size()) {
                ...
                row[c] = st.getStatus() != null && !st.getStatus().trim().isEmpty() ? st.getStatus() : "ناجح"; // حالة
        ...
        // Explicitly disable alternating row colors that some LookAndFeels force
        ...
        table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object val, boolean sel, boolean foc, int row,
                ...
                String plainForMeasure = arTxt.replaceAll("<[^>]*>", "");
                if (!arTxt.toLowerCase().startsWith("<html>")) {
                    arTxt = "<html><div dir='rtl' align='right' style='padding:15px 20px; word-wrap:break-word;'>" + arTxt
                            + "</div></html>";
                }
                Component comp = super.getTableCellRendererComponent(t, arTxt, false, false, row, col);
                comp.setBackground(Color.WHITE);
                comp.setForeground(Color.BLACK);
                // Auto-shrink font if text too wide for column
                int baseSize = 120, minSize = 60;
                int colW = t.getColumnModel().getColumn(col).getWidth() - 40;
                Font useFont = new Font("Tahoma", Font.PLAIN, baseSize);
                if (colW > 0 && !plainForMeasure.isEmpty()) {
                    java.awt.FontMetrics fm = t.getFontMetrics(useFont);
                    int sz = baseSize;
                    while (fm.stringWidth(plainForMeasure) > colW && sz > minSize) {
                        sz -= 2;
    ...
    private void styleTable(JTable table, int rowHeight) {
        ...
        table.getColumn("الحرفة").setPreferredWidth(2800);
    ...
    private JPanel buildPagePanel(List<Student> chunk, int pageNum, int totalPages) {
        ...
        int headerH = 1700, footerH = 800, tableHeaderH = 600; // Match actual preferred heights
        int available = panelHeight - headerH - footerH - tableHeaderH - 100; // Precision margin
        int totalRows = 17; // 2 stats rows + 15 student rows
        int calculatedH = available / totalRows;
        this.dynamicRowHeight = Math.min(1300, Math.max(120, calculatedH)); 
    ...
    public void createPDF() {
        try {
            String fn = "Detailed_Success_Report_" + profession.replace("/", "_").replace("\\", "_") + ".pdf";
    ...
    public void appendToDocument(Document doc) {
        try {
            ...
            java.util.concurrent.ExecutorService executor = java.util.concurrent.Executors.newFixedThreadPool(Math.max(2, cores / 2));
