...
/**
 * Report: Generic Graded Report — Precise Layout synchronized with successful report style (A3 Landscape).
 */
public class gradReportGeneric extends JFrame {
    ...
    public gradReportGeneric(String profession, String center, String region, String system, List<Student> students, String statusTitle, Color titleColor, String fileNamePrefix) {
    ...
    private JPanel buildHeader(int pageNum, int totalPages) {
        ...
        JLabel sub = label(statusTitle, 24, true);
    ...
    private JPanel buildTable(List<Student> chunk) {
        // --- Separate parent subjects from children (composite 30/70 subjects) ---
        ...
        // Display subjects = only parent subjects (composite ones show merged grade)
        int dispCount = parentSubjects.size();
        String[] cols = new String[9 + dispCount + 6];
        int i = 0;
        ...
        // Row 1: النهاية العظمى — sum children max if composite, else own max
        ...
        for (Subject s : parentSubjects) {
            int effectiveMax;
            if (childrenMap.containsKey(s.getId())) {
                effectiveMax = childrenMap.get(s.getId()).stream().mapToInt(Subject::getMaxMark).sum();
            } else {
                effectiveMax = s.getMaxMark();
            }
            maxRow[subIdx++] = effectiveMax;
            totalMax += effectiveMax;
            if ("نظري".equals(s.getType())) theoryMaxSum += effectiveMax;
            else if ("تطبيقي".equals(s.getType())) appliedMax = effectiveMax;
            else practicalMax = effectiveMax;
        }
        ...
        // Row 2: النهاية الصغرى — sum children pass if composite, else own pass
        ...
        for (Subject s : parentSubjects) {
            int effectivePass;
            if (childrenMap.containsKey(s.getId())) {
                effectivePass = childrenMap.get(s.getId()).stream().mapToInt(Subject::getPassMark).sum();
            } else {
                effectivePass = s.getPassMark();
            }
            minRow[subIdx++] = effectivePass;
            if ("نظري".equals(s.getType())) theoryPassSum += effectivePass;
            else if ("تطبيقي".equals(s.getType())) appliedPass = effectivePass;
            else practicalPass = effectivePass;
        }
            ...
            if (i_chunk < chunk.size()) {
                ...
                for (Subject sub : parentSubjects) {
                    ...
                    if (childrenMap.containsKey(sub.getId())) {
                        // Composite: sum grades of all children (30 + 70)
                        mark = childrenMap.get(sub.getId()).stream()
                                .mapToInt(child -> grades != null ? grades.getOrDefault(child.getId(), 0) : 0)
                                .sum();
                    } else {
