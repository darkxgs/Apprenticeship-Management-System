...
public class SystemSettingsPage extends JPanel {
    ...
    public SystemSettingsPage(AppFrame parent) {
        ...
        // --- FAIL-SAFE FIX FOR NEW DB BUG: FORCE COLUMN CREATION BEFORE LOADING ---
        try (Connection c = DatabaseConnection.getConnection(); Statement s = c.createStatement()) {
            s.execute("ALTER TABLE professions ADD (exam_system VARCHAR2(100))");
    ...
    private void addOrUpdateRegion() {
        ...
        String code = regionCodeField.getText().trim().replaceFirst("^0+", "");
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            // Check if code already exists for a different region
            try (PreparedStatement check = conn.prepareStatement(
                    "SELECT name FROM regions WHERE code = ? AND name != ?")) {
                check.setString(1, code);
                check.setString(2, name);
                try (ResultSet rs = check.executeQuery()) {
                    if (rs.next()) {
                        String existingRegion = rs.getString("name");
                        JOptionPane.showMessageDialog(this, 
                            "الكود \"" + code + "\" مستخدم بالفعل في المنطقة: " + existingRegion, 
                            "كود مكرر", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
            }
    ...
    private void addOrUpdateCenter() {
        ...
        String code = centerCodeField.getText().trim().replaceFirst("^0+", "");
        if (code.isEmpty()) code = "0";
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (regionId != null) {
                try (PreparedStatement check = conn.prepareStatement(
                        "SELECT name FROM centers WHERE code = ? AND region_id = ? AND name != ?")) {
                    check.setString(1, code);
                    check.setInt(2, regionId);
                    check.setString(3, name);
                    try (ResultSet rs = check.executeQuery()) {
                        if (rs.next()) {
                            String existingCenter = rs.getString("name");
                            JOptionPane.showMessageDialog(this, 
                                "الكود \"" + code + "\" مستخدم بالفعل في المركز: " + existingCenter + " في نفس المنطقة", 
                                "كود مكرر", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
