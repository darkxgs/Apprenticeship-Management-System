...
public class WithdrawalFramePage extends javax.swing.JFrame {
    
    private com.pvtd.students.ui.utils.ReportFilterPanel filterPanel;
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(WithdrawalFramePage.class.getName());
    ...
    private javax.swing.JPanel createStatsPanel() {
        ...
        // Using neutral blue-gray colors for Withdrawal
        statsContainer.add(createStatCard("إجمالي الطلاب", lblStatTotal, new java.awt.Color(71, 85, 105))); 
        statsContainer.add(createStatCard("المركز المختار", lblStatCenter, new java.awt.Color(51, 65, 85))); 
        statsContainer.add(createStatCard("المنطقة المختارة", lblStatRegion, new java.awt.Color(30, 41, 59))); 
    ...
    private void modernizeComponents() {
        ...
        buttonGradient1.setColor1(new java.awt.Color(71, 85, 105));
        buttonGradient1.setColor2(new java.awt.Color(30, 41, 59));
        buttonGradient1.setFont(UITheme.FONT_HEADER);
        buttonGradient1.setRadius(20);
        
        buttonGradientGrades.setText("📊 كشف بالدرجات");
        buttonGradientGrades.setColor1(new java.awt.Color(51, 65, 85));
        buttonGradientGrades.setColor2(new java.awt.Color(15, 23, 42));
        ...
        try {
            ...
            try (Connection con = DatabaseConnection.getConnection()) {
                String sql = "SELECT name, profession, registration_no, seat_no, status FROM students " +
                             "WHERE center_name = ? AND region = ? AND TRIM(status) = 'سحب الملف' ";
            ...
            lblStatTotal.setText(String.valueOf(model.getRowCount()));
            lblStatCenter.setText(center);
            lblStatRegion.setText(region);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("كشف التلاميذ مسحوبي الملفات");
        gridBagConstraints = new java.awt.GridBagConstraints();
        ...
        buttonGradient1.setText("كشف الطلاب مسحوبي الملفات بدون درجات");
        ...
        buttonGradientGrades.setText("كشف مسحوبي الملفات بالدرجات");
        ...
        layout.setHorizontalGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addComponent(jPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, 900, Short.MAX_VALUE));
    ...
    private void buttonSecretReportActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        ReportWorker worker = new ReportWorker(this, "كشف مسحوبي الملفات بالدرجات", null) {
            @Override
            protected Void doInBackground() throws Exception {
                ...
                try (Connection con = DatabaseConnection.getConnection()) {
                    ...
                    generatePdfFiles(studentsByRegion, centerName, selMonth, admMonth, "التقارير/تبييضة/مسحوبين");
                }
                java.awt.Desktop.getDesktop().open(new java.io.File("التقارير/تبييضة/مسحوبين"));
                return null;
            }
            ...
            private void generatePdfFiles(java.util.LinkedHashMap<String, java.util.List<com.pvtd.students.models.Student>> data, 
                ...
                for (java.util.Map.Entry<String, java.util.List<com.pvtd.students.models.Student>> entry : data.entrySet()) {
                    ...
                    if (mainRegion.isEmpty() && rKey != null && !rKey.isEmpty()) {
                        mainRegion = rKey;
                    }
                    fullCenterStudents.addAll(entry.getValue());
                }

                if (!fullCenterStudents.isEmpty()) {
                    // Sort all students by System then Seat Number
                    ...
                    gradReportWithdrawal report = new gradReportWithdrawal(center, mainRegion, fullCenterStudents, selM, admM);
    ...
    private void buttonGradient1ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        Withdrawal report = new Withdrawal(months[4], months[5]);
        if (report.isCancelled) return;

        DefaultTableModel model1 = (DefaultTableModel) jTable1.getModel();
        ReportWorker worker = new ReportWorker(this, "كشف طلاب مسحوبي الملفات", null) {
            @Override
            protected Void doInBackground() throws Exception {
                ...
                try (java.sql.Connection con = com.pvtd.students.db.DatabaseConnection.getConnection()) {
                    for (int i = 0; i < selectedRows.length; i++) {
                        ...
                        String seatNo = String.valueOf(model1.getValueAt(r, 1) != null ? model1.getValueAt(r, 1) : "").trim();
                        String systemName = "";
                        try (java.sql.PreparedStatement psSys = con.prepareStatement(
                                "SELECT s.exam_system FROM students s WHERE s.seat_no = ?")) {
                        ...
                        rowData.add(model1.getValueAt(r, 0)); // 0: status
                        rowData.add(model1.getValueAt(r, 1)); // 1: seatNo
                        rowData.add(model1.getValueAt(r, 2)); // 2: registration_no
                        rowData.add(String.valueOf(model1.getValueAt(r, 3)).trim()); // 3: profession
                        rowData.add(String.valueOf(model1.getValueAt(r, 4)).trim()); // 4: name
                        rowData.add(0); // 5: placeholder for index
                        sortedBySystem.computeIfAbsent(systemName, k -> new java.util.ArrayList<>()).add(rowData);
                    }
                }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new WithdrawalFramePage().setVisible(true));
    }
