...
public class StudentFormPage extends JPanel {
    ...
    private void setupDependencyListeners() {
        // 1. Region -> Centers (الفلترة الجديدة: اختيار المنطقة أولاً يملأ المراكز)
        regionCombo.addActionListener(e -> {
            Object sel = regionCombo.getSelectedItem();
            if (sel == null || sel.toString().trim().isEmpty()) return;
            String selRegion = sel.toString().trim();
            loadCentersForRegion(selRegion);
        });
        ...
        // 3. National ID → auto-fill DOB, gender, governorate
        nationalIdField.addFocusListener(new java.awt.event.FocusAdapter() {
            @Override
            public void focusLost(java.awt.event.FocusEvent e) {
                String nid = nationalIdField.getText();
                if (nid == null || nid.trim().length() != 14) return;
                
                com.pvtd.students.services.ValidationService.NationalIdInfo info = 
                    com.pvtd.students.services.ValidationService.validateNationalId(nid);
                if (info.valid) {
                    // Auto-fill DOB
                    if (dayCombo != null && info.dobDay != null) dayCombo.setSelectedItem(info.dobDay);
                    if (monthCombo != null && info.dobMonth != null) monthCombo.setSelectedItem(info.dobMonth);
                    if (yearCombo != null && info.dobYear != null) yearCombo.setSelectedItem(info.dobYear);
                    // Auto-fill gender
                    if (genderCombo != null && info.gender != null) genderCombo.setSelectedItem(info.gender);
                    // Auto-fill governorate
                    if (govCombo != null && info.governorate != null) govCombo.setSelectedItem(info.governorate);
                }
            }
        });
    }

    /** جلب المراكز المرتبطة بمنطقة معينة من جدول students */
    private void loadCentersForRegion(String region) {
        if (region == null || region.trim().isEmpty()) {
            centerNameCombo.removeAllItems();
            centerNameCombo.addItem("");
            centerNameCombo.setEnabled(false);
            return;
        }

        String currentCenter = centerNameCombo.getSelectedItem() != null
                ? centerNameCombo.getSelectedItem().toString() : "";
        centerNameCombo.removeAllItems();
        centerNameCombo.addItem("");
        
        java.util.Set<String> allCenters = new java.util.TreeSet<>();
        
        // 1. Load from centers table (official centers)
        try (java.sql.Connection conn = com.pvtd.students.db.DatabaseConnection.getConnection();
             java.sql.PreparedStatement stmt = conn.prepareStatement(
            ...
            try (java.sql.ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    if (centerName != null && !centerName.trim().isEmpty()) {
                        allCenters.add(centerName.trim());
                    }
                }
            }
        } catch (Exception ex) {
        ...
        // 2. Also load from students table (historical data)
        try (java.sql.Connection conn = com.pvtd.students.db.DatabaseConnection.getConnection();
             java.sql.PreparedStatement stmt = conn.prepareStatement(
                     "SELECT DISTINCT center_name FROM students WHERE region = ? AND center_name IS NOT NULL")) {
            stmt.setString(1, region);
            try (java.sql.ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
        ...
        // Add all centers to combo
        for (String center : allCenters) {
            centerNameCombo.addItem(center);
        }
        
        centerNameCombo.setEnabled(true);
        
        // إعادة الاختيار السابق لو كان موجوداً في القائمة الجديدة
        if (!currentCenter.isEmpty()) {
            centerNameCombo.setSelectedItem(currentCenter);
        }
    }
    ...
    private JPanel buildMainAcademicCard() {
        ...
        // Region Combo (يجب اختياره أولاً لفلترة المراكز)
        regionCombo = new JComboBox<>();
        regionCombo.setEditable(true);
        regionCombo.addItem("");
        for (String r : com.pvtd.students.services.DictionaryService.getCombinedItems(
                com.pvtd.students.services.DictionaryService.CAT_REGION)) {
            regionCombo.addItem(r);
        }
        addComboField(grid, "المنطقة (اختر أولاً)", regionCombo);

        // Center Name Combo (يتفلتر بعد اختيار المنطقة)
        centerNameCombo = new JComboBox<>();
        centerNameCombo.setEditable(true);
        centerNameCombo.addItem("");
        centerNameCombo.setEnabled(isEditMode); // Enabled only in edit mode if region exists, OR when region is selected
    ...
    private JPanel buildSecondaryAcademicCard() {
        ...
        // المنطقة انتقلت للكارت الأول جنب المركز
    ...
    private boolean validateGrades() {
        if (currentSubjectsList == null || currentSubjectsList.isEmpty()) return true;
        for (Subject sub : currentSubjectsList) {
            JTextField field = dynamicGradeFields.get(sub.getId());
            if (field != null) {
                String text = field.getText().trim();
                if (text.isEmpty() || text.startsWith("-")) continue;
                try {
                    int mark = Integer.parseInt(text);
                    if (mark > sub.getMaxMark()) {
                        JOptionPane.showMessageDialog(this,
                                "عفواً، الدرجة المُدخلة في مادة (" + sub.getName() + ") هي " + mark +
                                        " وهي أكبر من الدرجة العظمى المسموح بها " + sub.getMaxMark() + "!",
                                "خطأ في الدرجات", JOptionPane.ERROR_MESSAGE);
                        field.requestFocus();
                        return false;
                    }
                } catch (NumberFormatException ex) {
                    // Ignore non-numbers, collectGrades handles them
                }
            }
        }
        return true;
