...
/**
 * Report: تسويدة رصد درجات الدور الثاني
 * نسخة من تسويدة الدور الأول مع إظهار درجات الدور الأول (خلفية رمادية فاتحة)
 * وترك خلايا المواد الراسب فيها التلميذ فارغة (بيضاء) لكتابة درجة الدور الثاني يدوياً.
 * أعمدة المجاميع والملاحظات تُترك فارغة لإعادة حسابها بعد الدور الثاني.
 */
public class gradReportTasoedaSecondRound extends JFrame {

    /** لون خلفية خلايا درجات الدور الأول المعبأة */
    private static final Color FILLED_CELL_BG = new Color(0xD9D9D9);
    ...
    public gradReportTasoedaSecondRound(String profession, String center, String region, List<Student> students, boolean is3070,
        ...
        // تحميل درجات الدور الأول للطلاب الذين لم تُحمَّل درجاتهم مسبقاً (دفعة واحدة)
        ensureGradesLoaded();

        setTitle("تسويدة رصد درجات الدور الثاني - " + profession);
    ...
    /**
     * يجلب درجات الدور الأول من student_grades على دفعات (batch) للطلاب
     * الذين grades لديهم null — نفس نمط SecondRoundFailedFramePage.
     */
    private void ensureGradesLoaded() {
        ...
        try (Connection con = com.pvtd.students.db.DatabaseConnection.getConnection()) {
            for (int i = 0; i < missing.size(); i += batchSize) {
                ...
                List<Student> batch = missing.subList(i, end);
                ...
                try (PreparedStatement ps = con.prepareStatement(sb.toString())) {
                    for (int j = 0; j < batch.size(); j++) {
                        ps.setInt(j + 1, batch.get(j).getId());
                    }
    ...
    private JPanel buildTable(List<Student> chunk) {
        ...
        // New Column Order (بدون عمود حالة التلميذ):
        // م, الاسم, الحرفة, رقم التسجيل, رقم الجلوس, الرقم السري 1, " ", الرقم السري 2,
        // [Theory Subjects], مجموع النظري, العملي, تطبيقي, مجموع عملي وتطبيقي,
        // المجموع الكلي, ملاحظات
        int totalCols = 8 + theoryCount + 6;
        ...
        // خريطة الأب/الأبناء لتحديد الرسوب على مستوى المادة الأم (مجموع الأبناء)
        java.util.Map<Integer, Subject> subjectById = new java.util.HashMap<>();
        java.util.List<Subject> parentSubjects = new ArrayList<>();
        java.util.Map<Integer, List<Subject>> childrenMap = new java.util.HashMap<>();
        for (Subject s : allSubjects) {
            subjectById.put(s.getId(), s);
            if (s.getParentSubjectId() == null) {
                parentSubjects.add(s);
            } else {
                childrenMap.computeIfAbsent(s.getParentSubjectId(), k -> new ArrayList<>()).add(s);
            }
        }

        // خلايا درجات الدور الأول المعبأة (خلفية رمادية) — (صف، عمود)
        java.util.Set<java.awt.Point> grayCells = new java.util.HashSet<>();
        ...
        for (int i_chunk = 0; i_chunk < 10; i_chunk++) {
            ...
            int modelRow = 2 + i_chunk; // بعد صفي النهاية العظمى/الصغرى
            if (i_chunk < chunk.size()) {
                ...
                // مواد النظري: درجة الدور الأول (رمادية) إن كان ناجحاً في المادة،
                // وإلا خلية فارغة بيضاء لكتابة درجة الدور الثاني
                for (Subject colSub : theoryCols) {
                    Subject parent = (colSub.getParentSubjectId() != null)
                            ? subjectById.get(colSub.getParentSubjectId())
                            : colSub;
                    if (parent == null)
                        parent = colSub;
                    List<Subject> children = childrenMap.get(parent.getId());
                    int effMark = (children != null && !children.isEmpty())
                            ? children.stream()
                                    .mapToInt(ch -> grades != null ? grades.getOrDefault(ch.getId(), 0) : 0).sum()
                            : (grades != null ? grades.getOrDefault(parent.getId(), 0) : 0);
                    int effPass = (children != null && !children.isEmpty())
                            ? children.stream().mapToInt(Subject::getPassMark).sum()
                            : parent.getPassMark();
                    if (effMark >= effPass) {
                        row[colIdx] = grades != null ? grades.getOrDefault(colSub.getId(), 0) : 0;
                        grayCells.add(new java.awt.Point(modelRow, colIdx));
                    } else {
                        row[colIdx] = " ";
                    }
                    colIdx++;
                }
                row[colIdx++] = " "; // مجموع النظري (يُعاد حسابه بعد الدور الثاني)

                // عملي / تطبيقي: مجموع درجات الدور الأول على مستوى المادة الأم
                int practicalMark = 0, appliedMark = 0;
                boolean failedPractical = false, failedApplied = false;
                boolean hasPractical = false, hasApplied = false;
                for (Subject s : parentSubjects) {
                    boolean isPractical = "عملي".equals(s.getType()) || (s.getName() != null && s.getName().contains("عملي"));
                    boolean isApplied = !isPractical
                            && ("تطبيقي".equals(s.getType()) || (s.getName() != null && s.getName().contains("تطبيقي")));
                    if (!isPractical && !isApplied)
                        continue;
                    ...
                    if (isPractical) {
                        hasPractical = true;
                        practicalMark += effMark;
                        if (effMark < effPass) failedPractical = true;
                    } else {
                        hasApplied = true;
                        appliedMark += effMark;
                        if (effMark < effPass) failedApplied = true;
                    }
                }
                if (hasPractical && !failedPractical) {
                    row[colIdx] = practicalMark;
                    grayCells.add(new java.awt.Point(modelRow, colIdx));
                } else {
                    row[colIdx] = " ";
                }
                colIdx++; // العملي
                if (hasApplied && !failedApplied) {
                    row[colIdx] = appliedMark;
                    grayCells.add(new java.awt.Point(modelRow, colIdx));
                } else {
                    row[colIdx] = " ";
                }
        ...
        styleTable(table);
        table.setDefaultRenderer(Object.class, new TasoedaCellRenderer(grayCells));
    ...
    public int[] createPDF(com.itextpdf.text.Document combinedDoc, int startPageNum, int startStudentIdx) {
        ...
        try {
            ...
            File tasoedaFolder = new File(mainFolder, "تسويدة الدور الثاني");
    ...
    private class TasoedaCellRenderer extends DefaultTableCellRenderer {
        ...
        /** الخلايا المعبأة بدرجات الدور الأول (تُرسم بخلفية رمادية فاتحة) */
        private final java.util.Set<java.awt.Point> grayCells;

        TasoedaCellRenderer(java.util.Set<java.awt.Point> grayCells) {
        ...
        public java.awt.Component getTableCellRendererComponent(JTable table, Object val, boolean isSelected, boolean hasFocus, int row, int col) {
            ...
            boolean filled = grayCells != null && grayCells.contains(new java.awt.Point(row, col));
            comp.setBackground(filled ? FILLED_CELL_BG : Color.WHITE);
