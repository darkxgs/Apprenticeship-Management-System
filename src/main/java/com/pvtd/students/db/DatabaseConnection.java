...
public class DatabaseConnection {
    ...
    public static void initializeDatabase() {
        try (Connection conn = getConnection();
            ...
            // Subjects columns already in CREATE TABLE
            addColumnIfMissing(stmt, "subjects", "profession", "VARCHAR2(200)");
            addColumnIfMissing(stmt, "subjects", "specialization_id", "NUMBER");
            ...
            try (Statement s = conn.createStatement()) {
                // Check if any existing user has plain text password (doesn't contain ':'), if so, hash it
                try (ResultSet rs = s.executeQuery("SELECT username, password FROM users")) {
                    List<String[]> plainUsers = new ArrayList<>();
                    while (rs.next()) {
                        String uName = rs.getString("username");
                        String pwd = rs.getString("password");
                        if (pwd != null && !pwd.contains(":")) {
                            plainUsers.add(new String[]{uName, pwd});
                        }
                    }
                    if (!plainUsers.isEmpty()) {
                        try (PreparedStatement upStmt = conn.prepareStatement("UPDATE users SET password = ? WHERE username = ?")) {
                            for (String[] userPair : plainUsers) {
                                String uName = userPair[0];
                                String plainPwd = userPair[1];
                                String hashed = PasswordUtils.hashPassword(plainPwd);
                                upStmt.setString(1, hashed);
                                upStmt.setString(2, uName);
                                upStmt.executeUpdate();
                                System.out.println("Updated unhashed password for user: " + uName);
                            }
                        }
                    }
                }
            } catch (SQLException e) {
                System.err.println("Warning: failed to sanitize unhashed passwords: " + e.getMessage());
            }
                ...
                try (ResultSet rs = checkStmt.executeQuery()) {
                    if (!rs.next()) {
                        try (PreparedStatement insertStmt = conn.prepareStatement(
                            ...
                            insertStmt.setString(2, PasswordUtils.hashPassword("admin123"));
                ...
                try (ResultSet rs = checkStmt.executeQuery()) {
                    if (!rs.next()) {
                        try (PreparedStatement insertStmt = conn.prepareStatement(
                            ...
                            insertStmt.setString(4, "مستخدم تجريبي");
                            insertStmt.executeUpdate();
                            System.out.println("Seeded default test user.");
                        }
                    }
                }
            } catch (SQLException e) {
            ...
            System.out.println("✓ All sequences created/verified");
            System.out.println("✓ All triggers created/verified");
            System.out.println("✓ Default data seeded");
        } catch (SQLException e) {
