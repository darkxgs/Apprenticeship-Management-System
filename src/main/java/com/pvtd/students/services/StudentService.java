...
public class StudentService {
    ...
    public static Student getStudentBySecretNo(String secretNo) {
        String query = "SELECT * FROM students WHERE TRIM(secret_no) = ?";
        try (Connection conn = DatabaseConnection.getConnection();
                PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, secretNo.trim());
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    Student s = extractStudent(rs);
                    s.setGrades(getStudentGrades(conn, s.getId()));
    ...
    public static List<Student> getStudentsWithoutImages() {
        List<Student> students = new ArrayList<>();
        String query = "SELECT * FROM students WHERE image_path IS NULL OR TRIM(image_path) = '' " +
    ...
    public static String calculateStatus(String profession, Map<Integer, Integer> grades,
        ...
        // Rule 4: إذا لم يرسب في أي مادة -> تكون النتيجة ناجح
        return "ناجح";
    }
